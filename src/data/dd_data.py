...
def load_data(
    ...
    valid: bool = True,
    test: bool = False
) -> Tuple[Tuple[Optional[PromptDataset], Optional[PromptDataset], Optional[PromptDataset]], DiscreteDiffusionDataCollator]:
    ...
    if data_args.dataset_type in ["bilingual", "pair"]:
        ...
        if not hasattr(data_args, 'audio_encoder_name') or not data_args.audio_encoder_name:
            data_args.audio_encoder_name = getattr(model_args, 'audio_encoder_name', 'facebook/mms-300m')
        datasets = TranslatedSpeechDataset.load_data(data_args, tokenizer, train, valid, test)
