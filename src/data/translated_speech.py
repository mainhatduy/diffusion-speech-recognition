...
class TranslatedSpeechDataset(PromptDataset):
    ...
    def load_data(args, tokenizer, train=True, valid=True, test=False):
        ...
        print(
            f"Dataset split: {len(train_raw)} train, {len(valid_raw)} validation samples"
        )
        ...
        print(
            f"After filtering: {len(translated_dataset_train) if train else 0} train, {len(translated_dataset_valid) if (valid or test) else 0} validation samples"
        )

        train_dataset = (
            TranslatedSpeechDataset(
                args,
                translated_dataset_train,
                vietspeech_dataset,
                path_to_vs_idx,
                tgt_field,
                tokenizer,
