...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Submit Speech Translation models to Qualcomm AI Hub."
    )
    parser.add_argument(
        "--device",
        type=str,
        default="Samsung Galaxy S25 (Family)",
        help="Qualcomm target device/family name",
    )
    parser.add_argument(
        "--runtime",
        type=str,
        choices=["qnn", "onnx"],
        default="qnn",
        help="Target runtime (qnn or onnx)",
    )
    parser.add_argument(
        "--skip-repackage", action="store_true", help="Skip repackaging ONNX models"
    )
    ...
    for name, job in compile_jobs.items():
        ...
        try:
            profile_job = hub.submit_profile_job(
                model=target_model, device=device, name=f"{name}_profile"
            )
