...
def main():
    if len(sys.argv) < 2:
        ...
        print(
            "Example local: python scripts/model-manager/load_model.py outputs/vi_multitask/checkpoint-60000 test/test_data/test_sample.mp3"
        )
        print(
            "Example Hugging Face: python scripts/model-manager/load_model.py aiai-laboratory/discrete-diffusion-vi-multitask test/test_data/test_sample.mp3"
        )
        sys.exit(1)
    ...
    audio_encoder_name = getattr(
        model.config, "audio_encoder_name", "facebook/mms-300m"
    )
    if "moonshine" in audio_encoder_name.lower():
        ...
        audio_attention_mask = torch.ones_like(audio_values, dtype=torch.long).to(
            device
        )
