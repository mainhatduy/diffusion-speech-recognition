...
class DiscreteDiffusionDataArguments:
    dataset_type: str = field(
        default="bilingual"   # fairseq | flan | flanv2 | pair | bilingual
    )
    data_path: str = field(
        default=""
    )
...
class DiscreteDiffusionDataCollator(object):
    ...
    def __call__(self, samples):
        ...
        source_padded = torch.nn.utils.rnn.pad_sequence(
            sources, batch_first=True, padding_value=self.eos_id
        )
        target_padded = torch.nn.utils.rnn.pad_sequence(
            targets, batch_first=True, padding_value=self.eos_id
        )
