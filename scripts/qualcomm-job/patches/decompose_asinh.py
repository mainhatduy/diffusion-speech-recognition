...
def main():
    ...
    if asinh_node is None:
        print(
            "[!] No Asinh node found in the model graph. It might already be decomposed."
        )
        return

    print(
        f"[*] Found Asinh node: {asinh_node.name} at index {asinh_idx} with inputs {asinh_node.input} and outputs {asinh_node.output}"
    )
    ...
    one_tensor = helper.make_tensor(
        name="const_one_tensor", data_type=TensorProto.FLOAT, dims=[], vals=[1.0]
    )
    ...
    log_node = helper.make_node(
        "Log", inputs=["sum_val"], outputs=[output_var], name="decomposed_asinh_log"
    )
