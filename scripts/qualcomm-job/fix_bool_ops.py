    ...
    for idx, node in enumerate(graph.node):
        ...
        # 2. Fix GatherND (both BOOL data input and constant indices bug)
            ...
            if not indices_input_name.endswith("_indices_dynamic"):
                print(f"[*] {graph_name}: Found GatherND node '{node.name}' at index {idx} to make dynamic")
                ...
                zero_tensor = helper.make_tensor(
                    ...
                    dims=[],
                    vals=[0.0] if dynamic_input_type in (TensorProto.FLOAT, TensorProto.DOUBLE, TensorProto.FLOAT16) else [0]
                )
                ...
                if dynamic_input_type != indices_type:
                    ...
                    extra_nodes = [reduce_node, zero_const_node, mul_node, cast_zero_node]
                else:
                ...
                if t_type == TensorProto.BOOL:
                    print(f"[*] {graph_name}: Also fixing BOOL input type for GatherND node '{node.name}'")
                    ...
                    nodes_to_remove.append(idx)
                    nodes_to_insert.append((idx, extra_nodes + [add_node, cast_in, node, cast_out]))
