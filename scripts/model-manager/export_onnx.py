...
PretrainedConfig.from_pretrained = classmethod(
    patched_pretrained_config_from_pretrained
)
...
def main():
    ...
    print(
        f"Model backbone attention implementation: {model.model.config._attn_implementation}"
    )
    if hasattr(model, "audio_encoder"):
        print(
            f"Audio encoder attention implementation: {model.audio_encoder.config._attn_implementation}"
        )
    ...
    hidden_size = (
        model.audio_encoder.config.hidden_size
        if hasattr(model, "audio_encoder")
        else 1024
    )
