...
def main():
    ...
    if not index_path.exists():
        print(f"Error: {index_path} not found. Please run speech pre-computation first.")
        sys.exit(1)
    ...
    for shard_idx in range(num_shards):
        ...
        if shard_file.exists():
            print(f"Shard {shard_idx + 1}/{num_shards} ({shard_file.name}) already exists. Skipping.")
            continue
        ...
        print(f"Processing shard {shard_idx + 1}/{num_shards} ({len(shard_files)} files)...")
    ...
    if backup_dir.exists():
        print(f"Warning: Backup directory {backup_dir} already exists. Appending timestamp.")
