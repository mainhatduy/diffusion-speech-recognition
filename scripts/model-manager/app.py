...
def get_model_and_tokenizer():
    ...
    if _model is None:
        ...
        if config.tie_word_embeddings:
            _model.model.lm_head.decoder.weight = (
                _model.model.roberta.embeddings.word_embeddings.weight
            )
...
def load_audio(path: str, target_sr: int = 16000) -> np.ndarray:
    ...
    if waveform is None:
        raise RuntimeError(
            f"Could not load audio file '{path}'. Please upload a valid WAV, MP3, or FLAC file."
        )
...
def tokens_to_html_denoise_single_line(
    ...
    # Progress percent
    progress_percent = (
        int((step_idx / max_iterations) * 100) if max_iterations > 0 else 100
    )
    ...
    for i, c_id in enumerate(curr_tokens):
        if c_id == mask_id:
            ...
            if (
                ...
                and i < len(prev_tokens)
                and prev_tokens[i] == mask_id
            ):
                tokens_html.append(f'<span class="token token-new">{token_str}</span>')
            else:
                tokens_html.append(
                    f'<span class="token token-normal">{token_str}</span>'
                )
        ...
        with torch.no_grad():
            ...
            for lang in TASKS:
                ...
                denoise_htmls[lang] = tokens_to_html_denoise_single_line(
                    canvas_steps, tokenizer, 0, max_iterations
                )
            yield (
            ...
            for step_idx in range(max_iterations):
                ...
                for lang in TASKS:
                    ...
                    curr_translations[lang] = tokenizer.decode(
                        gen_tokens.cpu(), skip_special_tokens=True
                    ).strip()

                    canvas_steps = [[mask_id] * lang_state["canvas_len"]]
                    for step_tokens in lang_state["history"][1:]:
                        tokens_list = step_tokens[0][
                            ~lang_state["partial_mask"][0]
                        ].tolist()[: lang_state["canvas_len"]]
                        canvas_steps.append(tokens_list)

                    denoise_htmls[lang] = tokens_to_html_denoise_single_line(
                        canvas_steps, tokenizer, step_idx + 1, max_iterations
                    )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                load_sample_btn = gr.Button(
                    "📂 Use sample file (test_sample.mp3)", variant="secondary"
                )

                with gr.Accordion("Advanced Options", open=False):
                    iterations = gr.Slider(
                        minimum=5, maximum=50, value=10, step=1, label="Denoising steps"
                    )
                ...
                slow_mode = gr.Checkbox(
                    label="Slow mode (step-by-step visualization)", value=True
                )
