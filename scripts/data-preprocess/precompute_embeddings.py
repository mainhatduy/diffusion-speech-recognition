...
def parse_args():
    ...
    p.add_argument("--audio_encoder_name", type=str, default="UsefulSensors/moonshine-streaming-medium")
    ...
    p.add_argument("--task_tokens", nargs="+", default=["<vi_en>", "<vi_zh>", "<vi_ko>"])
    return p.parse_args()
...
class RawAudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if sr != self.target_sr:
            ...
            waveform = np.interp(np.linspace(0, len(waveform)-1, new_len), np.arange(len(waveform)), waveform)
        audio_inputs = self.feature_extractor(waveform, sampling_rate=self.target_sr, return_tensors="pt", padding=False)
        return {"audio_values": audio_inputs.input_values.squeeze(0), "path": audio_info.get("path", f"sample_{idx}"), "idx": idx}
...
def collate_audio(batch):
    ...
    for i, a in enumerate(audio_list):
        ...
        padded[i, :L] = a
        mask[i, :L] = 1
    return {"audio_values": padded, "attention_mask": mask, "paths": [b["path"] for b in batch], "indices": [b["idx"] for b in batch]}


def load_audio_encoder(name, cache_dir, device):
    if "moonshine" in name:
        from transformers import MoonshineStreamingModel
        encoder = MoonshineStreamingModel.from_pretrained(name, cache_dir=cache_dir).encoder
    else:
...
def precompute_audio(args, vietspeech_ds, feat_ext, path_to_vs_idx, out_dir, done_ids):
    ...
    loader = DataLoader(Subset(raw_ds, idxs), batch_size=args.batch_size, shuffle=False,
                        num_workers=args.num_workers, collate_fn=collate_audio, pin_memory=True,
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.pretrained, padding_side="right", use_fast=False, cache_dir=args.cache_dir)
    ...
    feat_ext = Wav2Vec2FeatureExtractor.from_pretrained(args.audio_encoder_name, cache_dir=args.cache_dir)

    from datasets import load_dataset, Audio
    translated_ds = load_dataset("aiai-laboratory/vietspeech-train-translated", token=hf_token, cache_dir=args.cache_dir, split="train[:100%]")
    vietspeech_ds = load_dataset("NhutP/VietSpeech", token=hf_token, cache_dir=args.cache_dir, split="train[:100%]")
    ...
    t0 = time.time()
    hs = precompute_audio(args, vietspeech_ds, feat_ext, path_to_vs_idx, args.output_dir, done_ids)
    ...
    vocab_hash = hashlib.md5(json.dumps(sorted(tokenizer.get_vocab().items())).encode()).hexdigest()
    meta = {"audio_encoder_name": args.audio_encoder_name, "audio_hidden_size": hs, "pretrained_model": args.pretrained,
            "tokenizer_vocab_size": len(tokenizer), "tokenizer_vocab_hash": vocab_hash, "max_length": args.max_length,
            "save_dtype": args.dtype, "task_tokens": args.task_tokens, "num_samples": len(translated_ds),
            "num_audio_files": len(path_to_vs_idx), "embed_stage": "before_projector"}
