...
def parse_args():
    parser = argparse.ArgumentParser(description="Download a full training checkpoint (including training state, optimizer, etc.) from Hugging Face Hub.")
    parser.add_argument(
        "--repo_id",
        type=str,
        required=True,
...
def main():
    ...
    try:
        ...
        checkpoints = [d for d in os.listdir(args.target_dir) if d.startswith("checkpoint-")]
        if checkpoints:
            checkpoints.sort(key=lambda x: int(x.split("-")[1]))
            highest_ckpt = checkpoints[-1]
            ckpt_path = os.path.join(args.target_dir, highest_ckpt)
            print(f"  python src/train.py <config_path> --resume_from_checkpoint {ckpt_path}")
        else:
            print(f"  python src/train.py <config_path> --resume_from_checkpoint {args.target_dir}")
