...
def run_pytorch(audio_path, repo_id, steps):
    ...
    print(
        f"    [+] Executing {steps}-step generation (prefix: {input_ids.tolist()[0]})..."
    )
...
def run_onnx(audio_path, repo_id, steps):
    ...
    # 5. Denoising Loop
    print(
        f"    [+] Executing {steps}-step generation (prefix: {output_tokens[0, :2].tolist()})..."
    )
...
def main():
    ...
    parser.add_argument(
        "--steps", type=int, default=10, help="Number of diffusion steps (default: 10)"
    )
    ...
    if args.mode == "both" and py_text is not None and on_text is not None:
        ...
        if match:
            print(
                "[+] MATCH SUCCESS: PyTorch and ONNX generated tokens match perfectly!"
            )
        else:
