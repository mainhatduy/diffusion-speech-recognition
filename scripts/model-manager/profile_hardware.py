...
def load_audio(path: str, target_sr: int = 16000) -> np.ndarray:
    ...
    if waveform is None:
        raise RuntimeError(
            f"Could not load audio file '{path}'. Please install soundfile or librosa."
        )
...
def run_inference_single(
    ...
    non_fixed_sym_masks = (
        canvas.ne(pad_id) & canvas.ne(bos_id) & canvas.ne(eos_id) & ~partial_mask
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Profile model hardware requirements for batch size 1."
    )
    parser.add_argument(
        "--repo-id",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Audio file to run benchmark on",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-markdown",
        type=str,
        default="scripts/model-manager/hardware_profile_report.md",
        help="Path to save markdown report",
    )
    parser.add_argument(
        "--output-json",
        type=str,
        default="scripts/model-manager/hardware_profile_report.json",
    ...
    print(
        f"Process RAM after loading model: {loaded_model_ram:.2f} MB (diff: {weights_ram_usage:.2f} MB)"
    )
    ...
    for threads in cpu_threads_to_test:
        ...
        print(
            f"  → Threads: {threads} | Latency: {avg_latency:.3f}s | RTF: {avg_rtf:.3f} | Speed: {avg_tokens_per_sec:.1f} tok/s | Peak RAM: {peak_rss_mb:.2f} MB"
        )
    ...
    if gpu_available:
        ...
        print(
            f"  → GPU Latency: {avg_latency:.3f}s | RTF: {avg_rtf:.3f} | Speed: {avg_tokens_per_sec:.1f} tok/s"
        )
    ...
    min_vram_mb = (
        (gpu_results["peak_vram_allocated_mb"] + 256.0) if gpu_results else 0.0
    )
    ...
    min_cores_for_rt = (
        ...
        else f"> {cpu_results[-1]['threads']} (requires GPU or optimization for real-time)"
    )
    ...
    report_dict = {
        ...
        "benchmarks": {"cpu": cpu_results, "gpu": gpu_results},
