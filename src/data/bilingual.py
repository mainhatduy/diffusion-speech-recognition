...
class BilingualDataset(PromptDataset):
    ...
    def __getitem__(self, index):
        ...
        tgt = self.tokenizer.encode(tgt_text, add_special_tokens=True)[
            -self.max_length :
        ]
    ...
    def load_data(args, tokenizer, train=True, valid=True, test=False):
        ...
        train_dataset = (
            BilingualDataset(args, datasets["train"], tokenizer)
            if train and "train" in datasets
            else None
        )
        valid_dataset = (
            BilingualDataset(args, datasets["validation"], tokenizer)
            if valid and "validation" in datasets
            else None
        )
        test_dataset = (
            BilingualDataset(args, datasets["test"], tokenizer)
            if test and "test" in datasets
            else None
        )
